...
# for aggreate account    
transfer_agg = np.zeros((num_country,num_country,num_time))
export = np.zeros((num_country,num_time))
for t in range(len(transfer[0,0,:])):
    for i in range(28):
        export[i,t] = np.sum(transfer[i,:,t])- np.sum(transfer[:,i,t])

for t in range(len(transfer[0,0,:])):
    for i in range(28):
        if export[i,t] >0: # >0 净出口
            transfer_agg[i,:,t] = export[i,t] *  transfer[i,:,t] / np.sum(transfer[i,:,t])
        if export[i,t] < 0: # 《0 净进口  
            transfer_agg[i,:,t] = 0
            
transfer = transfer_agg 
# total emission / kg        
ep = generation*ci_direct  
